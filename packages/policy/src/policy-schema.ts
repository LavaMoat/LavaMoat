    ...
    }),
    {
      error: (err) => {
        if (err.code === 'invalid_key') {
          return `Invalid canonical name; must be one or more package names delimited by ">" or ${ROOT}`
        }
      },
    }
  )
  .meta({
    description: `Mapping of canonical package name (or ${ROOT}) to package policy value`,
  })
...
const resourcesSchema = z
  ...
  .meta({
    description: `Mapping of canonical package name (or ${ROOT}) to resource policy value`,
    id: 'resources',
  })

/**
 * Schema for {@link LavaMoatPolicy.include}
 */
const includeSchema = z
  .array(
    z.union([
      canonicalNameSchema,
      z.object({ name: canonicalNameSchema, entry: z.string() }),
    ])
  )
  .optional()
  .meta({
    description: `List of canonical names (or ${ROOT}) and/or entry paths to forcefully include in the policy. Entries are relative to the package directory.`,
    id: 'include',
  })
 ...
 */
export const lavaMoatPolicySchema = z
  .object({
    resources: resourcesSchema,
    root: rootPolicySchema,
    include: includeSchema,
  })
  .refine(
    (value) =>
      value.root?.usePolicy !== undefined
        ? value.root.usePolicy !== ROOT &&
          !!value.resources[value.root.usePolicy]
        : true,
    {
      error: `If provided, root.usePolicy must reference a known resource (or ${ROOT})`,
      path: ['root', 'usePolicy'],
    }
  )
  .meta({
    description: 'LavaMoat policy',
  }) satisfies z.ZodType<LavaMoatPolicy>
