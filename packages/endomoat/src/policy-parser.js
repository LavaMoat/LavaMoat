...
/**
 *
 * @param {import('lavamoat-core').Resolutions} resolutions
 ...
 *
 * @param {URL} url
 * @returns {Pick<NodeJS.Require, 'resolve'>}
 */
const createRequire = (url) => {
  const phonyResolve = /** @type {NodeJS.RequireResolve} */ (
    (requestedName) => {
      ...
      if (resultMatchesRequest) {
        ...
        if (!looksLikeAPath && !isBuiltinModule) {
          ...
          const err = /** @type {NodeJS.ErrnoException} */ (new Error(errMsg))
...
/**
 * @typedef EMParseForPolicyOpts
 * @property {string} projectRoot
 * @property {string} entryId
 * @property {import('lavamoat-core').LavaMoatPolicyOverride} [policyOverride]
...
/**
 * @typedef {import('type-fest').Merge<import('lavamoat-core/src/parseForPolicy.js').ParseForPolicyOpts, EMParseForPolicyOpts>} ParseForPolicyOpts
 */
 ...
 * @returns
 */
function makeResolveHook(projectRoot, canonicalNameMap, resolutions = {}) {
  /**
   * @param {string} requestedName
  ...
  return (requestedName, referrer) => {
    ...
    try {
      ...
      if (
        /** @type {NodeJS.ErrnoException} */ (err).code === 'MODULE_NOT_FOUND'
      ) {
        console.warn(
...
/**
 * @typedef {import('type-fest').SetRequired<Pick<ParseForPolicyOpts, 'isBuiltin'|'resolveHook'|'policyOverride'|'shouldResolve'> & {canonicalNameMap: import('@lavamoat/aa').CanonicalNameMap}, 'isBuiltin' | 'resolveHook'>} MakeImportHookOpts
 */

/**
 *
 * @param {MakeImportHookOpts} opts
 * @returns {import('lavamoat-core/src/parseForPolicy.js').ImportHookFn}
 */
function makeImportHook({
  ...
  async function makeJsModuleRecord(specifier, filename, packageName) {
    ...
    // get imports
    const { cjsImports } = /** @type {{cjsImports: string[]}} */ (
      inspectImports(ast, null, false)
    )
    // build import map
