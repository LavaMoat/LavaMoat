 ...
 * point of entry.
 *
 * @example Metro.config.js const makeGetPolyfills =
 * require('@lavamoat/react-native-lockdown/makeGetPolyfills'); module.exports =
 * { serializer: { getPolyfills: ({platform}) => makeGetPolyfills({engine:
 * 'hermes', polyfills: ['reflect-metadata']}), }, };
 *
 * TODO: Implement ({platform: ?string}) from
 * https://metrobundler.dev/docs/configuration/#getpolyfills
 *
 * TODO: De-duplicate polyfills if '@react-native/polyfills' provided.
 *
 * NB: Sets up Hardened JS before
...
const makeGetPolyfills = ({ engine, polyfills }) => {
  if (polyfills === undefined) {
    const rnPolyfills = require('@react-native/polyfills')
    polyfills = rnPolyfills()
  }
  const ses =
