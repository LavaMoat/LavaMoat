...
function decorateWithPackageData(moduleData, canonicalNameMap) {
  ...
  if (moduleFilePath === undefined) {
    throw new Error(
      `module "${moduleData.id}" did not have a file path defined (moduleData.file)`
    )
  }
  ...
  if (!packageName) {
    throw new Error(
      `LavaMoat - invalid packageName for moduleId "${moduleData.id}"`
    )
  }
