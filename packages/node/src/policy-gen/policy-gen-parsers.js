 ...
 * generation.
 *
 * No transformations take place during policy generation other than those which
 * are required internally by `@endo/parser-pipeline`; only analysis occurs (see
 * {@link createPolicyGenVisitor}).
 *
...
export const createPolicyGenParsers = (inspectionResults, reporterOptions) => {
  ...
  if (reporterOptions) {
    ...
    ;({ concurrency } = reporterOptions)
    if ('reporter' in reporterOptions) {
      ;({ reporter, modulesToInspect, inspectedModules } = reporterOptions)
    }
  }
