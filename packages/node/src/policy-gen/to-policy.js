 ...
 * @param {Map<string, PackageJson>} packageJsonMap
...
export const buildModuleRecords = (
  ...
  const contexts = entries(compartments).reduce(
    (acc, [compartmentName, compartmentDescriptor]) => {
      ...
      let canonicalName
      if (compartmentName === entry.compartment) {
        canonicalName = LAVAMOAT_PKG_POLICY_ROOT
      } else if (compartmentName === ATTENUATORS_COMPARTMENT) {
        canonicalName = ATTENUATORS_COMPARTMENT
      }

      if (compartmentName in sources) {
        canonicalName ??= canonicalNameMap.get(compartmentName)
        if (!canonicalName) {
        ...
        const context = PolicyGeneratorContext.create(
          ...
          lmrCache,
          {
            packageJson: packageJsonMap.get(compartmentName),
            rootModule,
            readPowers,
            isBuiltin,
            log,
