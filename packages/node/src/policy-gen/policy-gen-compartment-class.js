...
const makeGetHints = (
  ...
  const getHints = (compartmentDescriptor) => {
    ...
    // gets the hints from `LavaMoatPolicy.hints`, if any
    const { hints: rawHints } = policyOverride
    if (rawHints?.[canonicalName]) {
      for (const hint of rawHints[canonicalName]) {
        hints.add(hint)
      }
    }
  ...
  class PolicyGenCompartment extends Compartment {
    ...
    constructor(options = {}) {
      ...
      const wrappedImportHook = async (specifier) => {
        ...
        getHints ??= makeGetHints(
          dataMap,
          makeGetValidCompartmentDescriptor(compartmentMap),
          { policyOverride, log }
        )

        const hints = getHints(compartmentDescriptor)
