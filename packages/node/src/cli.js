...
const main = async (args = hideBin(process.argv)) => {
   ...
   *
   * Resolves the entrypoint and determines if it is trusted. If it is trusted
   * and we're generating, a `ResourcePolicy` for the `entrypoint` package will
   * be generated.
   *
   * At runtime, we can cross-reference the `trust-entrypoint` flag with the
   * policy to ensure we do not accidentally run code that was supposed to be
   * untrusted as trusted.
   *
   * @remarks
