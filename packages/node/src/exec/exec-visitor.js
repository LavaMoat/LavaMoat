...
export const createExecVisitor = () => ({
  visitor: {
    Program: {
      /**
       * Remove the hashbang interpreter directive so @babel/generator does not
       * re-emit it (SES rejects sources that begin with `#!`), and defang any
       * `eval(` patterns lurking in comments attached to the enclosing `File`
       * node.
       *
     ...
     *
     * A direct eval call gives the callee access to the caller's lexical scope,
     * which undermines SES's scope isolation. Turning it into an indirect call
     * (i.e. not immediately preceded by a member-expression or direct
     * identifier reference) makes it operate in global scope instead.
     */
    CallExpression(path) {
    ...
    /**
     * Split string literals whose value would trip SES's direct-eval regex into
     * a `BinaryExpression` of two-or-more concatenated `StringLiteral`s.
     * Runtime semantics are preserved exactly; only the generated source layout
     * changes.
     *
     * Replacement nodes are themselves `StringLiteral`s, but each piece is — by
     * construction — split such that no piece can match the SES regex on its
     * own, so re-traversal is a safe no-op.
     */
    StringLiteral(path) {
