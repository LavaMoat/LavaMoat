...
import { resolveExecutable, resolveWorkspace } from '../../src/resolve.js'
...
test('resolveBinScript - resolves real bin script path', (t) => {
  ...
  const binPath = resolveExecutable('test-bin', { from: '/workspace', fs })
...
test('resolveBinScript - throws error if workspace not found', (t) => {
  ...
  t.throws(
    () => {
      resolveExecutable('test-bin', { from: '/nonexistent', fs })
    },
