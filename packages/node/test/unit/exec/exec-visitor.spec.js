...
/**
 * Test macro that transforms some code using our execution transforms and makes
 * assertions about the result.
 *
 * @type {Macro<[string, (t: ExecutionContext, result: string) => void]>}
 */
const testTransform = test.macro((t, source, assertions) => {
...
test(
  'leaves ordinary code unchanged',
  testTransform,
  'console.log("hello");',
  (t, result) => {
...
test(
  'rewrites direct eval to indirect eval',
  testTransform,
  'const result = eval("1 + 1");',
  (t, result) => {
    t.false(result.includes('eval("1 + 1")'), 'should not have direct eval')
    t.true(result.includes('(0, eval)'), 'should have indirect eval')
  }
)

test(
  'rewrites multiple eval calls',
  testTransform,
  'eval("foo"); eval("bar");',
  (t, result) => {
...
test(
  'does not rewrite method call eval',
  testTransform,
...
test(
  'defangs eval() inside line comments',
  testTransform,
...
test(
  'defangs eval() inside block / JSDoc comments',
  testTransform,
  [
    '/**',
...
test(
  'does not defang member-access eval inside comments',
  testTransform,
  `// see foo.eval(bar)\nvar x = 1;`,
  (t, result) => {
    t.true(
...
test(
  'does not touch comments without an eval pattern',
  testTransform,
  `// just a normal comment about evaluation\nvar x = 1;`,
  (t, result) => {
    t.true(
...
test(
  'still rewrites direct eval calls when comments are present',
  testTransform,
  ["// docs about 'eval()' calls", 'const result = eval("1 + 1");'].join('\n'),
  (t, result) => {
    t.notRegex(
      result,
      /(^|[^.])\beval\s*\(/,
...
test(
  'splits string literals containing eval() to defeat SES regex',
  testTransform,
  `var x = "eval()";`,
  (t, result) => {
    t.notRegex(
...
test(
  'preserves runtime value of split eval() string literal',
  testTransform,
  `var x = "eval()";`,
  (t, result) => {
...
test(
  'splits multiple eval() occurrences in a single string literal',
  testTransform,
  `var x = "eval() and eval(again)";`,
  (t, result) => {
    t.notRegex(
      result,
      /(^|[^.])\beval\s*\(/,
...
test(
  'does not split string literals without eval(',
  testTransform,
  `var x = "evaluate this expression";`,
  (t, result) => {
    t.true(
...
test(
  'does not split string literals with member-access eval',
  testTransform,
  `var x = "foo.eval(bar)";`,
  (t, result) => {
    t.true(
...
test(
  'handles string literals with eval() alongside live direct eval',
  testTransform,
  ['var bailout = "eval()";', 'var result = eval("1 + 1");'].join('\n'),
  (t, result) => {
    t.notRegex(
      result,
      /(^|[^.])\beval\s*\(/,
...
test(
  'escapes eval() inside bare template literals',
  testTransform,
  'var x = `eval()`;',
  (t, result) => {
    t.notRegex(
...
test(
  'preserves runtime cooked value of escaped template literal',
  testTransform,
  'var x = `eval()`;',
  (t, result) => {
    const recover = new Function(`${result}\nreturn x;`)
...
test(
  'escapes eval() in template quasis around interpolations',
  testTransform,
  'var x = `eval(${"foo"}) and eval(${"bar"})`;',
  (t, result) => {
    t.notRegex(
      result,
      /(^|[^.])\beval\s*\(/,
...
test(
  'does not touch tagged template literals',
  testTransform,
  'var x = String.raw`eval()`;',
  (t, result) => {
...
test(
  'does not touch templates without an eval pattern',
  testTransform,
  'var x = `evaluate this`;',
  (t, result) => {
    t.true(
...
test(
  'does not touch member-access eval inside templates',
  testTransform,
  'var x = `foo.eval(bar)`;',
  (t, result) => {
    t.true(
