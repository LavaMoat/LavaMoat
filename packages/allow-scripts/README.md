...
Adds a `.yarnrc` or `.npmrc` (the latter if `package-lock.json` is present) to the package, populates this file with the line `ignore-scripts true`.
...
Immediately after creating the configuration file, allow-scripts setup adds the devDependency `@lavamoat/preinstall-always-fail`. Adding this package to a project **mitigates** the likelihood of accidentally running any lifecycle scripts by throwing an error during the `preinstall` script execution.
...
If allowScripts has an item set to true, the lifecycle scripts of that package will be executed in the run phase if the current installed version is still matching.

> **Note** allow-scripts is pinning versions of the dependencies with lifecycle since v5. Items set to false will get automatically updated to the current version detected. To avoid churn in the list, you can remove the `#version` part from a disallowed item and it will be denied gardless of version. Versions are mandatory for allowed packages in case of maintainer compromise. You can opt out of versions with the `--skip-versions` flag.
...
Prints the complete set of information that allow-scripts uses to populate and run the allowlist content.
