(function (global, factory) {
  ...
  function rejectSomeDirectEvalExpressions(s) {
    ...
    if (index !== -1) {
      // monkey patch for overly strict
      const evalCommentPrelude = '// eslint-disable-line no-'
      const isEvalComment = evalCommentPrelude === s.slice(index - evalCommentPrelude.length, index)
      if (isEvalComment) {
        return
      }
      throw new SyntaxError(
        `possible direct eval expression rejected around line ${linenum} "${debugStart}"`
      );
    }
  }
